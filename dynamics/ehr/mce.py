...
class MultiEhrenfest(SimpleEhrenfest):
    ...
    def split_traj(self, mol: Molecule):
        temp = np.zeros_like(mol.coeff_s)
        temp[self._split] = mol.coeff_s[self._split]
        mol.coeff_s[self._split] = 0
        mol.coeff_s /= np.sqrt(1 - np.sum(np.abs(temp)**2))
        self._split = None

        clone = deepcopy(self)
        clone.mol.coeff_s = temp
        clone.mol.coeff_s /= np.sqrt(np.sum(np.abs(temp)**2))
        return clone

    def update_nuclear(self, mols: list[Molecule]):
        self._phase += 0.5 * mols[-1].kinetic_energy * self.dt
        temp = super().update_nuclear(mols)
        self._phase += 0.5 * mols[-1].kinetic_energy * self.dt
        return temp
